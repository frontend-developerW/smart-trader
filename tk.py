...
class TraderBotGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("🧠 Smart Binance Bot (Multi-Timeframe, WebSocket)")
    ...
    def run_bot(self, api_key, api_secret, profit_percent):
        try:
            ...
            trending = self.get_strong_trending_coins()
            slots = 4
    ...
    def get_strong_trending_coins(self):
        self.log("🔄 Analyzing market for strong coins across multiple timeframes...")
        try:
            ...
            for sym in usdt_symbols:
                try:
                    ...
                    for tf in timeframes:
                        klines = self.client.get_klines(symbol=sym, interval=tf, limit=2)
            ...
            return [x[0] for x in sorted(trending, key=lambda x: x[1], reverse=True)[:10]]
        except Exception as e:
